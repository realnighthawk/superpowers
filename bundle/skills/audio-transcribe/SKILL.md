...
**Never install whisper, ffmpeg, or any local packages. Use the HTTP API via exec.**
...
1. Transcribe via the LiteLLM proxy:
```bash
curl -s "$LITELLM_BASE_URL/audio/transcriptions" \
